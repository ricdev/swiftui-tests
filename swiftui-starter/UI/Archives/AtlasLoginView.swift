...
//        }
    ...
    private func useTouchFaceID() {
        UserDefaults.standard.set(viewModel.email, forKey: "usernameKey")
        UserDefaults.standard.set(viewModel.password, forKey: "passwordKey")
...
//        self.loginUsingV2()
    ...
    private func storeUserProfileImageUrl(profileImageUrl: URL) {
        guard let uid = FirebaseManager.shared.auth.currentUser?.uid else { return }
        let userData = ["email": viewModel.email, "uid": uid, "profileImageUrl": profileImageUrl.absoluteString]
